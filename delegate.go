...
func DelegateOIDtoString(oid asn1.ObjectIdentifier) string {
	if oid.Equal(OID_delegateOnboard) {
		return "onboard"
	}
	if oid.Equal(OID_delegateUpload) {
		return "upload"
	}
	if oid.Equal(OID_delegateRedirect) {
		return "redirect"
	}
...
func DelegateStringToOID(str string) (asn1.ObjectIdentifier, error) {
	switch {
	case str == "onboard":
		return OID_delegateOnboard, nil
	case str == "upload":
		return OID_delegateUpload, nil
	case str == "redirect":
		return OID_delegateRedirect, nil
	case str == "claim":
		return OID_delegateClaim, nil
	case str == "provision":
		return OID_delegateProvision, nil
	default:
		return OID_delegateBase, fmt.Errorf("Invalid Delegate OID string")
