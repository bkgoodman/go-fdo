...
start_server() {
    ...
    go run ./examples/cmd server -http "$SERVER_ADDR" -db "./$DB_FILE" $flags > /tmp/fdo_server.log 2>&1 &
    SERVER_PID=$!
...
test_basic() {
    ...
    run_cmd go run ./examples/cmd client -di "$SERVER_URL"
    log_success "DI completed"
    
    log_step "Running TO1/TO2 (Transfer Ownership)"
    run_cmd go run ./examples/cmd client
...
test_rv_blob() {
    ...
    if go run ./examples/cmd client -rv-only 2>&1 | grep -q "ERROR"; then
    ...
    run_cmd go run ./examples/cmd server -to0 "$SERVER_URL" -to0-guid "$GUID" -db "./$DB_FILE"
    log_success "RV blob registered"
    
    log_step "Verifying TO1 now succeeds"
    run_cmd go run ./examples/cmd client -rv-only
...
test_kex() {
    ...
    run_cmd go run ./examples/cmd client -di "$SERVER_URL" -di-key rsa2048
    log_success "DI completed with RSA2048"
    
    log_step "Running TO1/TO2 with ASYMKEX2048"
    run_cmd go run ./examples/cmd client -kex ASYMKEX2048
...
test_fdo200() {
    ...
    run_cmd go run ./examples/cmd client -fdo-version 200
...
test_delegate() {
    ...
    run_cmd go run ./examples/cmd server -http "$SERVER_ADDR" -db "./$DB_FILE" -owner-certs &
    SERVER_PID=$!
    sleep 2
    stop_server
    
    log_step "Creating delegate chain"
    run_cmd go run ./examples/cmd delegate -db "$DB_FILE" create myDelegate onboard,redirect SECP384R1 ec384 ec384
    log_success "Delegate chain created"
    
    log_step "Listing delegate chains"
    run_cmd go run ./examples/cmd delegate -db "$DB_FILE" list
    
    log_step "Printing delegate chain"
    run_cmd go run ./examples/cmd delegate -db "$DB_FILE" print myDelegate
    ...
    run_cmd go run ./examples/cmd server -db "$DB_FILE" -to0 "$SERVER_URL" -rvDelegate myDelegate -to0-guid "$GUID"
